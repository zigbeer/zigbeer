...
describe("APIs Arguments Check for Throwing Error", function() {
  ...
  describe("#.parse", function() {
    ...
    it("should throw TypeError if input clusterId is not a number and not a string", function() {
      ...
      expect(() => zcl.parse(buf, function() {}, rethrow)).toThrowError(
        TypeError
      )

      expect(() => zcl.parse(buf, "genOnOff", function() {})).not.toThrowError(
        TypeError
      )
      expect(() => zcl.parse(buf, 6, function() {})).not.toThrowError(TypeError)
    })
  })
