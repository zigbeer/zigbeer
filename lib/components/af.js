...
af.zclFunctional = function (srcEp, dstEp, cId, cmd, zclData, cfg, callback) {
    ...
    if (srcEp instanceof Group) {
        af.sendExt(srcEp, ZSC.AF.addressMode.ADDR_GROUP, srcEp.getGroupID(), cId, zclBuffer).fail(function (err) {
            if (mandatoryEvent && areq.isEventPending(mandatoryEvent))
                areq.reject(mandatoryEvent, err);
            else
                deferred.reject(err);
    ...
    switch (type) {
        ...
        case 'zclIncomingMsg':
            ...
            if (frameType === 0 && msg.zclMsg.cmdId === 'report')
                af.controller.getShepherd().emit('ind:reported', targetEp, msg.clusterid, msg.zclMsg.payload, msg);
            ...
            if (frameType === 1 && cmdIDs.includes(msg.zclMsg.cmdId) && msg.zclMsg.payload) {
                af.controller.getShepherd().emit('ind:cmd', targetEp, msg.clusterid, msg.zclMsg.payload, msg.zclMsg.cmdId, msg);
            }
